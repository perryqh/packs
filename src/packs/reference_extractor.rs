...
use super::{checker::reference::Reference, file_utils::expand_glob, Configuration, SourceLocation};
...
fn autoload_paths_from_config(configuration: &Configuration) -> HashMap<PathBuf, String> {
...
pub(crate) fn get_all_references_new(
    configuration: &Configuration,
    _absolute_paths: &HashSet<PathBuf>,
) -> anyhow::Result<Vec<Reference>> {
    ...
    let ref_config = ruby_references::configuration::Configuration {
        ...
        included_files: configuration.included_files.clone(),
